...
引用计数法为每个Java对象添加一个**引用计数器**，引用失效计数器就 - 1，有新的引用就 + 1，通过计数器的数值来判断该对象是否是可回收对象。
...
引用计数法就因为很难解决对象之间相互**循环引用**的问题，所以目前JVM是采用可达性分析算法来判断对象是否是可回收对象。
...
![绘图](E:\z-relax\JavaGetOffer\JVM专栏\JVM垃圾回收\绘图.png)

可以作为GC Roots对象一共包括以下四种：
...
![绘图](E:\z-relax\JavaGetOffer\JVM专栏\JVM垃圾回收\绘图1.png)
...
![绘图](E:\z-relax\JavaGetOffer\JVM专栏\JVM垃圾回收\绘图2.png)
